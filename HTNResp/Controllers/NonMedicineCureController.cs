...
{
    public class NonMedicineCureController : BaseController
    {
        HTNResp.BLL.NonMedicineCure bll = new HTNResp.BLL.NonMedicineCure();
        ...
        public override ActionResult AjaxList()
        {
            ...
            List<HTNResp.Model.NonMedicineCure> list = bll.DataTableToList(bll.GetListByPage(strSql, "", start + 1, start + length).Tables[0]);
        ...
        {
            Model.NonMedicineCure model = bll.GetModel(id);
        ...
        [Login(Feedback = 100)]
        public ActionResult Update(Model.NonMedicineCure model)
        {
            bool result = false;
            HTNResp.BLL.Program bllProgram = new BLL.Program();
            HTNResp.BLL.EvalGuid bllEvalGuid = new BLL.EvalGuid();
